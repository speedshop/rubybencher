...
class RunTest < ActiveSupport::TestCase
  ...
  test "validates presence of required fields" do
    ...
    assert_includes run.errors[:tasks_per_instance_type], "can't be blank"
  end

  test "validates tasks_per_instance_type is positive" do
    run = Run.new(ruby_version: "3.4.7", tasks_per_instance_type: 0)

    assert_not run.valid?
    assert_includes run.errors[:tasks_per_instance_type], "must be greater than 0"
  end
  ...
  test "current scope returns most recent running run" do
    old_run = Run.create!(ruby_version: "3.4.7", tasks_per_instance_type: 3)
    old_run.complete!
    sleep 1

    current_run = Run.create!(ruby_version: "3.4.8", tasks_per_instance_type: 3)
